...
/*
  ...
  */
  const addCars = (car, brand) => {
    const checkCar = utils.cars.find((cars) => cars.id === car.id);
    const checkBrand = utils.brands.find((brands) => brands.name === brand.name);

    if (checkCar) throw Error("Este coche ya existe en la base de datos");
    if (checkBrand) throw Error("Esta marca ya existe en la base de datos");

    if(checkCar === undefined && checkBrand === undefined){
