...
router.get("/cars/:brandName", (req, res) => {
   ...
   const { brandName } = req.params;
    ...
    const cars = getCarsByBrand(brandName, sort);
    if (cars === "No se encontraron coches") {
      return res.status(400).json({ error: cars });
    }

    return res.status(200).json({ brand: brandName, results: cars });
  } catch (err) {
